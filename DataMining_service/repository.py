...
class ReviewRepository:
    ...
    async def get_reviews_stats(self) -> dict:
        ...
        source_query = select(Review.source, func.count(Review.uuid)).group_by(
            Review.source
        )
        ...
        ratings_stats = {
            row[0].value if row[0] else None: row[1] for row in rating_result.fetchall()
        }
    ...
    async def get_reviews_stats_by_region(self) -> dict:
        ...
        query = (
            select(
                Review.region_code, Review.city, func.count(Review.uuid).label("count")
            )
        ...
        for row in result.fetchall():
            ...
            if region_code not in stats:
                stats[region_code] = {"total_reviews": 0, "cities": {}}
