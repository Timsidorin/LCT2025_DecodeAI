...
class DashboardService:
    ...
    async def get_regions_sentiment_heatmap_filtered(
        ...
        regions_data = (
            await self.analytics_repo.get_regions_with_filtered_sentiment_heatmap(
        ...
        if regions_data:
            ...
            total_target_reviews = sum(
                r["target_sentiment_count"] for r in regions_data
            )
            total_all_reviews = sum(r["total_reviews"] for r in regions_data)

            # Топ регионы по целевому sentiment
            top_regions = sorted(
                regions_data, key=lambda x: x["target_sentiment_count"], reverse=True
            )[:5]

            # Регионы с высоким процентом
            high_percentage_regions = [
                r for r in regions_data if r["target_sentiment_percentage"] > 70
            ]
            ...
            analytics = {
                ...
                "average_target_percentage": round(
                    sum(r["target_sentiment_percentage"] for r in regions_data)
                    / total_regions,
                    2,
                ),
                "top_regions": [
        ...
        response = {
            ...
            "color_scheme": color_schemes.get(
                sentiment_filter, color_schemes["positive"]
            ),
            "analytics": analytics,
    ...
    async def get_regional_dashboard(
        self, region_code: Optional[str] = None, limit: int = 20
    ) -> Dict[str, Any]:
        base_tasks = [
        ...
        if region_code:
            ...
            base_tasks.extend(
                [
                    ...
                    self.analytics_repo.get_daily_trends(
                        days_back=14, filters=region_filters
                    ),
        ...
        if region_code and len(results) > 2:
            ...
            dashboard_data["region_recommendations"] = (
                await self._generate_region_recommendations(results[2], results[4])
            )
    ...
    async def get_all_regions(
        self, include_cities: bool = False, min_reviews: int = 0
    ) -> Dict[str, Any]:
    ...
    async def get_regions_sentiment_map(
        self, min_reviews: int = 0, include_cities: bool = False
    ) -> Dict[str, Any]:
        ...
        if regions_data:
            total_regions = len(regions_data)
            positive_regions = len(
                [r for r in regions_data if r["sentiment_score"] > 0.1]
            )
            negative_regions = len(
                [r for r in regions_data if r["sentiment_score"] < -0.1]
            )
            ...
            analytics = {
                ...
                "neutral_regions_count": total_regions
                - positive_regions
                - negative_regions,
    ...
    async def _get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage": {"regions_covered": regions_count, "data_completeness": 95.5},
        }

    async def _generate_regional_insights(
        self, regional_stats: List[Dict]
    ) -> List[Dict[str, Any]]:
        ...
        if regional_stats:
            ...
            positive_regions = [
                r for r in regional_stats if r.get("positive_rate", 0) > 60
            ]
            if positive_regions:
                insights.append(
                    {
    ...
    async def _generate_region_recommendations(
        self, sentiment_data: Dict[str, int], region_info: Optional[Dict]
    ) -> List[Dict[str, Any]]:
        """Рекомендации на основе анализа региона"""
        recommendations = []

        if sentiment_data and sentiment_data.get("negative", 0) > sentiment_data.get(
            "positive", 0
        ):
    ...
    async def _get_activity_heatmap(self, minutes_back: int) -> Dict[str, Any]:
        ...
        return {"heatmap_data": [], "peak_periods": [], "low_activity_periods": []}

    async def _analyze_recent_activity(
        self, recent_reviews: List[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_active_regions": sorted(
                regions_activity.items(), key=lambda x: x[1], reverse=True
            )[:3],
            "activity_score": len(recent_reviews) / 20 * 100,
        }

    async def _calculate_region_summary(
        self, sentiment_data: Dict[str, int], cities_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        sentiment_summary = {
            "dominant_sentiment": (
                max(sentiment_data, key=sentiment_data.get)
                if sentiment_data
                else "unknown"
            ),
            "positive_rate": round(
                (
                    (sentiment_data.get("positive", 0) / total_reviews * 100)
                    if total_reviews > 0
                    else 0
                ),
                2,
            ...
            "negative_rate": round(
                (
                    (sentiment_data.get("negative", 0) / total_reviews * 100)
                    if total_reviews > 0
                    else 0
                ),
                2,
        ...
        cities_summary = {
            ...
            "cities_with_reviews": len(
                [city for city in cities_data if city["count"] > 0]
            ),
        }
