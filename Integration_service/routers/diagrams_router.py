...
async def get_regions_sentiment_heatmap(
    ...
    try:
        ...
        heatmap_data = await dashboard_service.get_regions_sentiment_heatmap_filtered(
            sentiment_filter=sentiment_value, min_reviews=min_reviews
        )
...
async def get_regions_sentiment_map(
    ...
    try:
        return await dashboard_service.get_regions_sentiment_map(
            min_reviews=min_reviews, include_cities=include_cities
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
