...
def save_with_retry(df, base_filename):
    ...
    attempts = [
        ...
        os.path.join(
            os.path.expanduser("~"), "Desktop", f"dataset_cleaned_{timestamp}.csv"
        ),
        os.path.join(
            os.path.expanduser("~"), "Downloads", f"dataset_cleaned_{timestamp}.csv"
        ),
    ]

    for filename in attempts:
        try:
            df.to_csv(filename, encoding="windows-1251", index=False, sep=";")
...
def main():
    ...
    print(
        f"   Экономия за счет кэша: {((total_cities - unique_cities) / total_cities * 100):.1f}%"
    )
    ...
    batches = [
        cities_list[i : i + batch_size] for i in range(0, len(cities_list), batch_size)
    ]
    ...
    print(
        f"💾 Статистика кэша: попаданий {cache_info.hits}, промахов {cache_info.misses}"
    )
